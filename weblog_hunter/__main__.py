...
Examples:
  ...
  %(prog)s --input logs/ --format all --top 20 --min-req 100
    ...
    # Input/Output
    parser.add_argument(
        "--input", required=True, help="Path to access.log or folder of logs (.log/.gz supported)"
    )
    parser.add_argument(
        "--out", default="report.md", help="Output report path (default: report.md)"
    )
    parser.add_argument(
        "--json", dest="json_out", default=None, help="Optional JSON report output path"
    )
    parser.add_argument(
        "--html", dest="html_out", default=None, help="Optional HTML report output path"
    )
    parser.add_argument(
        "--format",
        dest="output_format",
    ...
    # Analysis options
    parser.add_argument(
        "--top", type=int, default=10, help="Top N suspicious IPs to detail (default: 10)"
    )
    parser.add_argument(
        "--min-req",
        type=int,
        default=50,
        help="Minimum requests before scoring an IP (default: 50)",
    )

    # Config file
    parser.add_argument(
        "--config", dest="config_file", default=None, help="Configuration file (YAML)"
    )
    ...
    entries, failures, files_read = log_parser.parse_logs(
        args.input, show_progress=config.show_progress and not config.quiet
    )
