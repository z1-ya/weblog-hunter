...
from weblog_hunter.models import AnalysisResult, EndpointVulnerability, IPAnalysis, LogEntry
...
class ThreatAnalyzer:
    ...
    def _analyze_ip(self, ip: str, events: List[LogEntry]) -> IPAnalysis:
        ...
        # Get tools used
        tools = list({e.tool for e in events if e.tool})
    ...
    def _rank_vulnerable_endpoints(self, entries: List[LogEntry]) -> List[EndpointVulnerability]:
        ...
        for endpoint, stats in endpoint_stats.items():
            score = 3 * stats["sqli_hits"] + 2 * stats["sqli_500"] + len(stats["payloads"])
    ...
    def _infer_scrape_section(
        self, by_ip: Dict[str, List[LogEntry]], top_ips: List[str]
    ) -> Optional[str]:
        """Infer which section is being used for email/data scraping"""
        scrape_candidates = defaultdict(
            lambda: {"hits": 0, "ok": 0, "bytes": [], "paths": Counter()}
        )
        ...
        for _ip, sc in scrape_candidates.items():
            if sc["hits"] == 0:
                continue
